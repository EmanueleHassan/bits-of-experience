...
#+end_export
     ...
     points sampled from the true distribution. Where as for the true
     distribution we will use the *sampled distribution* as an
     approximation.
     ...
     it is to the unknown optimum.

     Notice moreover that in our discussion we will be interested in
     the /likelihood/ of the data given the model M $l(D : M)$. (nota
     bene la notation).

     Another option for comparing how well a model fits a distribution
     is through the notion of /loss functions/ $l(\xi : M)$. This

...
*** Check if one variable in enum
    ...
    There are apparently libraries to do that: check [[EnumUtils.isValidEnum(MyEnum.class, myValue)][at this]].
...
** TODO Strings and working with them                              :noexport:

   check for instance the =StringBuilder= method.
...
public void test() throws SQLException {
    // Single connection insert
    
    final String MYTEST = "" + "insert into result.Factor_Loading" +
	"(as_of_version, securities, credit_factor, coefficient)" + "values" +
	"(?, ?, ?, ?)" + "";
      
    connInsert.execute(MYTEST, asof().asofVersionID, "hello", "hello", 3.0);
    connInsert.commit();
...
*** TODO understand why prepared statment vs. connection.execute

    
*** TODO write about the logic of committing the results and executing them :noexport:
   
*** TODO make notes about general logic of JDBC                    :noexport:

...
class question {
	...
	x = h <= 4 + 2 ? 2 : 3;  // ok so note that you must
...
*** List of Optional elements
    ...
    Note that you can as well create list of Optional Objects. This
    might be important in your queries when writing them.    
	...
	public static void main(String args[]) {
	    ...
	    List<Optional<String>> myList = new ArrayList<>();

	    Optional<String> opt = Optional.empty();

	    Optional<String> opt2 = Optional.of("baeldung");

	    myList.add(opt); myList.add(opt2); 

	    for (Optional<String> optional : myList) {
...
*** Count Occurrences in a List
	...
	public static void main(String args[]) {
	    ...
	    myList.add(opt); myList.add(opt2); myList.add(opt3);

	    int occurrences = Collections.frequency(myList, "hello");

	    System.out.println(occurrences);

	    // Just get the first - if not 5 then it is not what you want.
...
* Java Beans
  ...
  class instances you define in a predefined way.

  For example, if a library wants to stream any object you pass into
  it, it knows it can because your object is serializable (assuming
  the library requires your objects be proper JavaBeans).

  
** Beans in Spring

   Do not confuse them with Spring beans. You see that these are by
   nature quite different. 

   Anyways the idea is that you can inject then these Beans throughout
   the applicaiton.
...
* Garbage Collection
  ...
  that you created. It is nice how there is a lot biblical concepts
  over there as well.
...
** On Naming Convention
   ...
   The point of naming convention is also interesting.

   You will note when working on java projects that most start with
   the com. Or that part of it is part of their package name.

   The reason for it is [[https://stackoverflow.com/questions/4620045/why-is-the-root-package-in-source-code-called-com][explained here]]. You should use your
   <domainname>.domain, in /reverse order/.

   So for instance =com.zurich=.
...
* TODO Profiling
  ...
  It will be good to check at it at some point.
